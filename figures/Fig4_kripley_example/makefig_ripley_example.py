...
def plot_forecasts_sims():
    ...
    sim_cat.plot(ax=Ax, plot_args=sim_props, extent=region.get_bbox())
    pts = region.tight_bbox()
    ax.plot(pts[:, 0], pts[:, 1], lw=1, color='black',
            transform=cartopy.crs.PlateCarree())
    plt.savefig('forecast_a.png', dpi=600,
    ...
    ax.plot(pts[:, 0], pts[:, 1], lw=1, color='black',
            transform=cartopy.crs.PlateCarree())
    plt.savefig('forecast_b.png', dpi=600,
                facecolor=(0, 0, 0, 0), bbox_inches='tight', pad_inches=0)
...
def plot_l_funcs(alpha=0.05, figsize=(6, 4)):
    ...
    plt.savefig(f'L_models.png', dpi=900, facecolor=(0, 0, 0, 0))
